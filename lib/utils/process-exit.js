...
module.exports = function processExit(projectPath, projectName, appType) {
  ...
  var allStoriesFiles = glob.sync(projectPath+"/src/**/*.stories.js", {})
  var allReadme = glob.sync(projectPath+"/src/styles/**/README.md", {})
  var allPackages = glob.sync(projectPath+"/src/styles/**/package.json", {})

  // Remove all *.stories.js files
  try {
    allStoriesFiles.forEach(file => { fse.removeSync(file) })
  } catch (e) {/* Noop */}

  try { fs.unlinkSync(path.join(projectPath, 'src', 'README.md')) } catch (e) { /* Noop */ }
  try { fs.unlinkSync(path.join(projectPath, 'src', 'package.json')) } catch (e) { /* Noop */ }
  try { fs.unlinkSync(path.join(projectPath, 'src', 'CHANGELOG.md')) } catch (e) { /* Noop */ }
  try { fs.unlinkSync(path.join(projectPath, 'src', '.git')) } catch (e) { /* Noop */ }
  try { fs.unlinkSync(path.join(projectPath, 'src', '.prettierrc')) } catch (e) { /* Noop */ }
  try { fs.unlinkSync(path.join(projectPath, '.gitmodules')); } catch (e) { /* Noop */ }

  // Remove README and package.json in styles folder
  try {
    allReadme.forEach(file => { fse.removeSync(file) })
  } catch (e) {/* Noop */}
  try {
    allPackages.forEach(file => { fse.removeSync(file) })
  } catch (e) {/* Noop */}
