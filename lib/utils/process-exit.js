...
module.exports = function processExit(projectPath, projectName, appType) {
  ...
  try {
    var allStoriesFiles = glob.sync("src/**/*.stories.js", {})
    var mappedFiles = allStoriesFiles.map(file => path.resolve(__dirname+'/'+file))
    mappedFiles.forEach(file => fse.removeSync(file))
  } catch (e) {/* Noop */}
